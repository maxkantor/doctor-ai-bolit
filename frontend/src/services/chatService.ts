...
export const chatService = {
  ...
  async getRemainingMessages(visitorId: string): Promise<number | { remainingMessages: number; creditBalance: number; freeMessagesRemaining: number; messageCount: number }> {
    console.log('🔍 Fetching remaining messages for visitorId:', visitorId)
    const response = await api.get<{ remainingMessages: number; creditBalance?: number; freeMessagesRemaining?: number; messageCount?: number }>(`/chat/remaining-messages?visitorId=${visitorId}`)
